...
platforms.each { |platform|
  ...
  puts "version number #{platform.version_number}"
  p platform.extensions
...
#  puts platform.create_context_from_type( OpenCL::Device::Type::GPU ).devices.first.name
  ...
  if context.devices.first.image_support == OpenCL::TRUE then
    p context.supported_image_formats( OpenCL::Mem::IMAGE2D )
    p context.create_image_2d( OpenCL::ImageFormat::new( OpenCL::ChannelOrder::RGBA, OpenCL::ChannelType::UNSIGNED_INT8), 128, 25)
  end
  puts b_in.size
  puts b_in.flags
  puts b_in.type
